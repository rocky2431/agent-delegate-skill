...
def _validate_target_record(name: object, target: object) -> None:
    ...
    if target.get("zcode_runtime") and (len(argv) < 2 or
            target.get("adapter_path") != argv[1] or not Path(argv[1]).is_absolute()):
        raise DelegationError(f"Target {name!r} must bind its zCode adapter_path to argv[1].")
...
def _runtime_identity(target: dict[str, Any], acpx: str, probe: bool = True) -> dict[str, Any]:
    ...
    if target.get("adapter_package"):
        adapter = _package_identity(target.get("adapter_path", target["argv"][0]), target["adapter_package"])
...
def _launch(args: argparse.Namespace) -> int:
    ...
    if target.get("zcode_runtime"):
        try:
            target, env = zcode_runtime.prepare(target, env)
        except zcode_runtime.ZCodeRuntimeError as exc:
...
def _doctor(args: argparse.Namespace) -> int:
    ...
    for name in names:
        try:
            ...
            if target.get("zcode_runtime"):
                target, zcode_env = zcode_runtime.prepare(target, dict(os.environ))
                ready, detail = zcode_runtime.probe(target, zcode_env)
                checks.append({"check": f"target:{name}:app_server_ready", "ok": ready, "detail": detail})
        except (DelegationError, zcode_runtime.ZCodeRuntimeError) as exc:
